...
/**
 * Auto-commit changes to Git and optionally push
 */
async function autoCommitChanges(task: string, push: boolean = false): Promise<{ success: boolean; sha?: string; error?: string }> {
...
Co-Authored-By: Claude Opus 4.5 <noreply@anthropic.com>`;
    ...
    // Chain: git add, commit, and optionally push
    const commands = push
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, task, reason, toolName, toolDescription, parameters, implementation, autoCommit = true, autoPush = false } = body;
...
Please make the necessary code changes to accomplish this task.`;
        ...
        runClaudeCode(prompt, {
          ...
          if (result.success) {
            ...
            if (autoCommit) {
              const commitResult = await autoCommitChanges(task, autoPush);
...
Please implement this new tool.`;
        ...
        runClaudeCode(prompt, {
          ...
          if (result.success) {
            ...
            // Auto-commit if enabled
            if (autoCommit) {
              const commitResult = await autoCommitChanges(`Add tool: ${toolName}`, autoPush);
              if (commitResult.success) {
                commitSha = commitResult.sha;
              }
            }
      ...
      case 'commit': {
        ...
        const result = await autoCommitChanges(message || 'Manual self-improvement commit', push);
        return NextResponse.json({
