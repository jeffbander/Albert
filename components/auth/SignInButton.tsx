 ...
 *
 * A button that initiates the OAuth sign-in flow.
...
export function SignInButton({
  ...
  const getButtonStyle = () => {
    const baseStyle = 'flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors';

    if (provider === 'google') {
      return `${baseStyle} bg-white text-gray-700 border border-gray-300 hover:bg-gray-50`;
    }
    if (provider === 'github') {
      return `${baseStyle} bg-gray-900 text-white hover:bg-gray-800`;
    }
...
/**
 * Sign In Buttons Group
 *
 * Displays all available sign-in options.
 */
